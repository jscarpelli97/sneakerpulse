...
/** Upsert a local mirror of a cloud account (no password required). */
export function mirrorCloudAccount(input: {
  email: string;
  username: string;
  ...
  const existing = vault.accounts[key];
  vault.accounts[key] = {
    email: key,
    username: input.username.trim().toLowerCase() || existing?.username || "collector",
    salt: existing?.salt ?? "",
  ...
  writeVault(vault);
  setSession({ email: key, username: vault.accounts[key].username });
  return true;
}
